...
def sgRNAs(path):
    ...
    # fourth try catch to calculate combination of 10 sgRNAs and write out the ranked guides
    try:
    ...
    # fifth try catch to read in the chosen guides and write out the primer sequences
    try:
        # read in the common sgRNAs and thier genome locations
        outputpath= path+'\output_input-guides.txt'
        df = pd.read_csv(outputpath, sep='\t')
