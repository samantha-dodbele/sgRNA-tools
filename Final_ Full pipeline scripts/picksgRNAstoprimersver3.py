...
def sgRNAsver3(path):
    ...
    try:
        ...
        # condition if there are between 20 to 29 guides in common then removes guides within 5 nt
        elif 20 <= len < 30:
            startiter = 0
            delete = []
            for i in range(1, len):
        ...
        # condition if there are more than 30 guides in common then removes guides within 8 nt
        elif len >= 30:
            startiter = 0
            delete = []
            for i in range(1, len):
                if start.loc[i] - start.loc[startiter] <= 8:
                    delete.append(i)
                else:
                    startiter = i
    ...
    try:
        ...
        sheet1.write(0, 1, "Primers (5' to 3')")

        # populate original guide sequence and prepped and checked Twist library primer in sheet
        length = guides.size
        ...
        for x in range(0, length):
            ...
            prepped = prep(seq)
            if check_char(prepped) == True:
                sheet1.write(x+1, 1, get_primer(prepped))
            else:
                sheet1.write(x+1, 1, "No primer ")
